...
export const POST = withErrorHandler("Error creating booking", async (req: NextRequest) => {
  const rateLimited = applyRateLimit(req, "booking", 10, 60 * 60 * 1000);
  if (rateLimited) return rateLimited;

  const { data, error } = await parseBody(req, bookingSchema);
